...
export function SitesPage() {
  ...
  return <section className="page page-wide">
    ...
    {!query.isPending && !query.isError && sites.length > 0 && <>
      <div className="metric-grid metric-grid-three"><MetricCard label="Sites suivis" value={sites.length} detail="périmètre courant" tone="good" /><MetricCard label="Presses référencées" value={machineCount || 'N/D'} detail="catalogue machine" /><MetricCard label="Incidents ouverts" value={openIncidentCount || 'N/D'} detail="à examiner" tone={openIncidentCount ? 'danger' : 'good'} /></div>
      ...
      <div className="site-grid">{sites.map((site) => <article className="site-card" key={site.id}>
        ...
        <button className="button-secondary site-open" type="button" onClick={() => navigate(`/sites/${site.id}/workshop`)}>Ouvrir l’atelier <ArrowRightIcon size={17} aria-hidden="true" /></button>
      </article>)}</div>
    </>}
  </section>;
}
