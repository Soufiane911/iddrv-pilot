...
import { defectTypeLabel, EmptyPanel, formatDate, incidentSeverityLabel, incidentStatusLabel, incidentSymptomLabel, SectionTitle, StatePanel } from '../components/Ui';
...
export function IncidentsPage() {
  ...
  function resetFilters() { changeStatus('all'); }
  return <section className="page page-wide">
    ...
    {!query.isPending && !query.isError && incidents.length > 0 && <section className="surface-card incident-table-card"><SectionTitle eyebrow="FILE DE DIAGNOSTIC" title="Signaux récents" /><div className="incident-table-wrap"><table className="incident-table"><caption className="visually-hidden">Liste des incidents persistés</caption><thead><tr><th scope="col">Incident</th><th scope="col">Machine</th><th scope="col">Gravité</th><th scope="col">Statut</th><th scope="col">Début</th><th scope="col"><span className="visually-hidden">Action</span></th></tr></thead><tbody>{incidents.map((incident) => <tr key={incident.id}><td data-label="Incident"><Link className="incident-link" to={`/incidents/${incident.id}`}><strong>{incidentSymptomLabel(incident.symptom)}</strong><small>{defectTypeLabel(incident.defect_type)} · {incident.id.slice(0, 8)}</small></Link></td><td data-label="Machine"><span className="machine-ref">{incident.machine_erp_ref ? `Presse ${incident.machine_erp_ref}` : `Machine ${incident.machine_id}`}</span><small>Site {incident.site_id}</small></td><td data-label="Gravité"><span className={`severity severity-${incident.severity}`}>{incidentSeverityLabel(incident.severity)}</span></td><td data-label="Statut"><span className={`status-label status-label-${incident.status}`}>{incidentStatusLabel(incident.status)}</span></td><td data-label="Début"><time dateTime={incident.started_at}>{formatDate(incident.started_at)}</time></td><td data-label="Action"><Link className="table-action" to={`/incidents/${incident.id}`}>Ouvrir <ArrowRightIcon size={16} aria-hidden="true" /></Link></td></tr>)}</tbody></table></div></section>}
  </section>;
}
