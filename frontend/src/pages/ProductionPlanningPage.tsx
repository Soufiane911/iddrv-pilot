...
function readable(error: unknown) { if (error instanceof ApiRequestError) { const labels: Record<string, string> = { week_start_required: 'La semaine sélectionnée est invalide.', press_slot_conflict: 'Un autre OF occupe déjà cette presse sur cet intervalle.', stale_row_version: 'Ce créneau a été modifié. Actualisez avant de réessayer.', slot_cancelled: 'Ce créneau est déjà annulé.', site_not_found: 'Ce site n’est plus disponible.', work_order_machine_not_found: 'Cette presse ne fait pas partie de l’OF.', scrap_value_invalid: 'Le rebut réel doit être un entier positif ou vide.' }; return labels[error.code] ?? error.message; } return error instanceof Error ? error.message : 'Le planning est indisponible.'; }

export function ProductionPlanningPage() {
  const { siteId: siteIdParam } = useParams();
  const api = useApi();
  const [params, setParams] = useSearchParams();
  const routeSiteId = Number(siteIdParam);
  const querySiteId = Number(params.get('siteId'));
  const [selectedSiteId, setSelectedSiteId] = useState(Number.isFinite(querySiteId) ? querySiteId : NaN);
  const independent = !Number.isFinite(routeSiteId);
  const siteId = Number.isFinite(routeSiteId) ? routeSiteId : selectedSiteId;
  const navigate = useNavigate();
  const cache = useQueryClient();
  const sites = useQuery({ queryKey: ['sites'], queryFn: api.getSites, enabled: independent });
  useEffect(() => {
    if (independent && !Number.isFinite(selectedSiteId) && sites.data?.length) {
      const first = sites.data.find(site => site.status !== 'archived') ?? sites.data[0];
      if (first) { setSelectedSiteId(first.id); const next = new URLSearchParams(params); next.set('siteId', String(first.id)); setParams(next, { replace: true }); }
    }
  }, [independent, params, selectedSiteId, setParams, sites.data]);
  ...
  const canEnterScrap = !siteArchived && (role === 'operator' || canPlan);
  ...
  const scrap = useMutation({ mutationFn: ({ slot, value }: { slot: PlanningSlot; value: number | null }) => api.updateWorkOrderScrap(slot.workOrderId, { actual_scrap_count: value, machine_id: slot.machineId, row_version: slot.actualScrapRowVersion ?? 1 }), onSuccess: async () => { setError(null); await cache.invalidateQueries({ queryKey: ['planning', siteId] }); }, onError: value => { setError(readable(value)); void cache.invalidateQueries({ queryKey: ['planning', siteId] }); } });
  ...
  if (independent && sites.isPending) return <section className="page"><StatePanel tone="loading" title="Chargement des sites" text="Sélection du site à planifier." /></section>;
  if (independent && sites.isError) return <section className="page"><StatePanel tone="error" title="Sites indisponibles" text="Impossible de récupérer les sites pour le planning." action="Réessayer" onAction={() => sites.refetch()} /></section>;
  if (!Number.isFinite(siteId)) return <section className="page"><StatePanel tone="error" title="Aucun site sélectionné" text="Sélectionnez un site pour ouvrir son planning." /></section>;
  ...
  return <section className="page page-wide planning-page"><div className="page-intro"><div><p className="eyebrow">ATELIER · PLANNING PRODUCTION</p><h2>{site.data?.name ?? 'Planning hebdomadaire'}</h2><p className="muted">Une ligne par créneau OF-presse. Les cycles machine, la collecte et l’ERP restent des états indépendants.</p></div>{independent ? <label className="planning-site-picker">Site<select aria-label="Site du planning" value={siteId} onChange={event => { const nextSiteId = Number(event.target.value); setSelectedSiteId(nextSiteId); const next = new URLSearchParams(params); next.set('siteId', String(nextSiteId)); setParams(next, { replace: true }); }} >{(sites.data ?? []).map(item => <option key={item.id} value={item.id} disabled={item.status === 'archived'}>{item.name}</option>)}</select></label> : <button className="button-secondary" type="button" onClick={() => navigate('/sites')}>Changer de site</button>}</div>{site.isError && <StatePanel tone="error" title="Site indisponible" text={readable(site.error)} action="Réessayer" onAction={() => site.refetch()} />}<PlanningContent timezone={planning.data?.timezone ?? site.data?.timezone ?? 'UTC'} week={week} filters={filters} setFilters={setFilters} machines={activeMachines} machinesError={machinesQuery.isError} canPlan={canPlan} canEnterScrap={canEnterScrap} canImport={canImport} onWeekChange={shift} onCurrentWeek={currentWeek} onImport={() => navigate(`/imports?siteId=${siteId}`)} onCreate={() => { setEditing(null); setEditorOpen(true); }} refreshedAt={refreshedAt} loading={planning.isFetching} planning={planning} items={items} onEdit={slot => { setEditing(slot); setEditorOpen(true); }} onCancel={cancelSlot} onSaveScrap={(slot, value) => scrap.mutate({ slot, value })} scrapBusyId={scrap.isPending ? scrap.variables?.slot.id ?? null : null} error={error} onRetry={() => planning.refetch()} /><ProductionAssignmentEditor siteId={siteId} timezone={planning.data?.timezone ?? site.data?.timezone ?? 'UTC'} machines={activeMachines} slot={editing} open={editorOpen} canPlan={canPlan} onClose={() => setEditorOpen(false)} onSaved={async () => { setEditorOpen(false); setError(null); await cache.invalidateQueries({ queryKey: ['planning', siteId] }); }} /></section>;
}

function PlanningContent({ timezone, week, filters, setFilters, machines, machinesError, canPlan, canEnterScrap, canImport, onWeekChange, onCurrentWeek, onImport, onCreate, refreshedAt, loading, planning, items, onEdit, onCancel, onSaveScrap, scrapBusyId, error, onRetry }: {  timezone: string; week: string; filters: PlanningFilters; setFilters: (value: PlanningFilters) => void; machines: Machine[]; machinesError: boolean; canPlan: boolean; canEnterScrap: boolean; canImport: boolean; onWeekChange: (delta: number) => void; onCurrentWeek: () => void; onImport: () => void; onCreate: () => void; refreshedAt?: Date; loading: boolean; planning: { isPending: boolean; isError: boolean; error: unknown; data?: PlanningWeek; refetch: () => unknown }; items: PlanningSlot[]; onEdit: (slot: PlanningSlot) => void; onCancel: (slot: PlanningSlot) => void; onSaveScrap: (slot: PlanningSlot, value: number | null) => void; scrapBusyId: string | null; error: string | null; onRetry: () => void }) {
