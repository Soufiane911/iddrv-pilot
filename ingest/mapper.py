        ...
        if isinstance(value, str):
            ...
            if value.lower() in {"", "nan", "nat", "n/a", "na", "null", "none"}:
                ...
                try:
                    ...
                    pass  # Garder comme string si non numérique
...
if __name__ == "__main__":
    ...
    # We import loader dynamically to avoid circular import issues if any
    import loader
    ...
    try:
        ...
        # Output logic
        if args.db_url and not args.buffer:
            ...
            cursor.execute("INSERT INTO import_passports (file_name, file_hash, status) VALUES (%s, %s, 'completed') RETURNING id", (Path(input_path).name, "dummy_hash_" + str(time.time()),))
            ...
            try:
                ...
                for r in rows:
                    ...
                    if machine_key not in machine_ids:
                        cursor.execute("SELECT id FROM machines WHERE erp_ref = %s", (machine_key,))
                ...
                insert_sql = """
                    INSERT INTO machine_cycles (
                        time, machine_id, passport_id, cycle_time_s, peak_pressure_bar, cushion_mm
                    ) VALUES (%s, %s, %s, %s, %s, %s)
                """
                for offset in range(0, len(insert_values), batch_size):
                    psycopg2.extras.execute_batch(
                        cursor,
                        insert_sql,
                        insert_values[offset:offset + batch_size],
        ...
        elif args.buffer == "redis" and args.redis_url:
            # Buffer in Redis
            import redis
            r_conn = redis.Redis.from_url(args.redis_url)
            for r in rows:
                # Add E2E aliases
                out_row = r.copy()
                if "time" in r:
