...
API_TABLE_GRANTS: Mapping[str, frozenset[str]] = {
    ...
    "sites": frozenset({"SELECT", "UPDATE"}),
    "production_lines": frozenset({"SELECT"}),
    "machines": frozenset({"SELECT", "INSERT", "UPDATE"}),
    ...
    # Workshop planning (020), site gateway receipts (021) and lifecycle
    # controls (022).  Archive and source administration are API writes;
    # ERP/receipt workers use the same tables through their own matrix.
    "work_orders": frozenset({"SELECT", "INSERT", "UPDATE"}),
    "work_order_allocations": frozenset({"SELECT", "INSERT", "UPDATE"}),
    "planning_slots": frozenset({"SELECT", "INSERT", "UPDATE"}),
    "planning_audit_events": frozenset({"SELECT", "INSERT"}),
    "site_sources": frozenset({"SELECT", "INSERT", "UPDATE"}),
