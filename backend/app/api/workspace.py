...
from ..repositories import ImportLifecycleConflict, add_import_file, create_import_session, get_import_session, validate_import_session
...
def create_session(site_id: int, payload: ImportSessionRequest, identity: Identity = Depends(get_current_identity)):
    ...
    except ImportLifecycleConflict as exc:
...
def validate_session(session_id: UUID, identity: Identity = Depends(get_current_identity)):
    ...
    except (ImportLifecycleConflict, ValueError) as exc:
        if str(exc) in {"import_session_not_profiled", "site_archived"}:
            raise HTTPException(status_code=409, detail=str(exc)) from None
        raise
