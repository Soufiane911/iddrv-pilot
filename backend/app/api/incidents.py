...
from ..security import Identity, get_identity_optional, require_roles, require_site
...
def investigate(incident_id: UUID, as_of: datetime | None = None,
    ...
    inc = get_incident(incident_id, allowed_site_ids=None if identity.is_admin else identity.site_ids)
...
def feedback(incident_id: UUID, payload: FeedbackRequest,
             identity: Identity = Depends(require_roles("analyst", "supervisor", "admin"))):
    if get_incident(incident_id, allowed_site_ids=None if identity.is_admin else identity.site_ids) is None:
