...
from ..site_repository import SiteConflict, SiteNotFound, create_site, delete_site
...
def remove_site(site_id: int, response: Response, identity: Identity = Depends(require_roles("admin"))):
    # Admin is global governance for destructive tenant operations.  The
    # repository still locks and rechecks the site before deleting it.
    try:
        delete_site(site_id)
        if site_id in identity.site_ids:
            roles = tuple((value, role) for value, role in identity.site_roles if value != site_id)
            refreshed = Identity(identity.user_id, identity.email, identity.display_name, identity.role, tuple(value for value, _ in roles), identity.session_id, site_roles=roles)
            token, expires_at = create_session_token(refreshed)
            if identity.session_id and replace_session_token(identity.session_id, token, expires_at):
                set_session_cookie(response, token)
    except SiteNotFound:
