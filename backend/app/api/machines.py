...
def _machine_or_404(machine_id: int, as_of: datetime | None = None):
    value = get_machine(machine_id, as_of=as_of) if as_of is not None else get_machine(machine_id)
    if value is None:
...
def machine(
    ...
    value = _machine_or_404(machine_id, as_of)
